		...
		if lastProcessed == "" {
			...
			for i := len(runes) - 1; i >= 0; i-- {
				for _, ending := range sentenceEndings {
					if runes[i] == ending {
						// 更新到句子结束标记之后（包含标记）
						m.lastProcessedCumulativeText = string(runes[:i+1])
						return newSentences
					}
				}
			}
			...
			// 计算已处理文本的结束位置（上次处理的文本 + 新提取的句子）
			// newSentences 是从 lastProcessed 之后提取的，所以应该是 lastProcessed + newSentences
...
// extractNewSentences 从累积文本中提取新的完整句子（包含句子结束标记的部分）
func (m *Manager) extractNewSentences(current string) string {
	if current == "" {
		return ""
	}
	...
	sentenceEndings := []rune{'。', '！', '？', '.', '!', '?'}
	for _, ending := range sentenceEndings {
		if strings.ContainsRune(newText, ending) {
			// 找到第一个句子结束标记的位置
			runes := []rune(newText)
			for i, r := range runes {
				if r == ending {
