...
import { Key, Settings, AppWindow, ChevronDown, RefreshCw, ArrowRight, Bot, MessageCircle, Users, Zap, Circle, Phone, Plus, X, Edit2, ExternalLink } from 'lucide-react';
...
interface ControlPanelProps {
  ...
  // SIP配置
  assistantId: number
  trunkNumbers: Array<{id: number, number: string, direction: string, status: string}>
  scriptPlans: Array<{id: number, name: string}>
  associatedNumbers: Array<{id: number, numberId: number, scriptPlanId: number, number: string, scriptPlanName: string}>
  onRefreshTrunkNumbers: () => void
  onRefreshScriptPlans: () => void
  onAssociateNumber: (numberId: number, scriptPlanId: number) => void
  onUnlinkNumber: (numberId: number) => void
  onCreateScriptPlan?: (name: string, description: string, scriptId?: string, scriptConfig?: string) => Promise<void>
  onUpdateScriptPlan?: (id: number, name: string, description: string, scriptId?: string, scriptConfig?: string) => Promise<void>
...
const ControlPanel: React.FC<ControlPanelProps> = ({
  ...
  scriptPlans,
  associatedNumbers,
  onRefreshTrunkNumbers,
  onRefreshScriptPlans,
  onAssociateNumber,
  onUnlinkNumber,
  onCreateScriptPlan,
  onUpdateScriptPlan,
  ...
  return (
    <div className={cn('flex-1 p-6 overflow-y-auto space-y-4 custom-scrollbar', className)}>
      <div className="space-y-6 min-h-0 max-h-[85vh]">
                  ...
                  <div>
                    ...
                    <Select value={selectedJSTemplate || ""} onValueChange={onJSTemplateChange}>
                      <SelectTrigger 
                        ...
                        selectedValue={
                          selectedJSTemplate ? 
                            jsTemplates.find(t => t.jsSourceId === selectedJSTemplate)?.name || t('controlPanel.sip.unknown')
                          ...
                          <SelectItem key={template.id} value={template.jsSourceId} className="px-3 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer">
                            <div className="flex items-center gap-2">
                              <span className={cn(
                                ...
                                {template.type === 'default' ? t('controlPanel.sip.default') : t('controlPanel.sip.custom')}
                              </span>
        ...
        >
          <SectionHeader
            title={t('controlPanel.sip.title')}
            icon={<Phone className="w-5 h-5" />}
            section="sip"
          />
          
          <AnimatePresence>
            {expandedSections.sip && (
              <motion.div
                initial={{ height: 0, opacity: 0 }}
                animate={{ height: 'auto', opacity: 1 }}
                exit={{ height: 0, opacity: 0 }}
              ...
              >
                <div className="pt-4 space-y-4">
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                      {t('controlPanel.sip.associatedNumbers')}
                    </label>
                    {associatedNumbers.length > 0 ? (
                      <div className="space-y-2">
                        {associatedNumbers.map((item) => (
                          <div
                            key={item.id}
                          ...
                          >
                            <div className="flex-1">
                              ...
                              <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                {t('controlPanel.sip.scriptPlan')} {item.scriptPlanName}
                              </div>
                            </div>
                            <button
                              onClick={() => onUnlinkNumber(item.numberId)}
                              className="ml-2 px-3 py-1 text-xs text-red-600 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-900/20 rounded transition-colors"
                            >
                              {t('controlPanel.sip.unlink')}
                            </button>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <div className="p-3 text-sm text-gray-500 dark:text-gray-400 text-center bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                        {t('controlPanel.sip.noNumbers')}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-4 pt-4 border-t dark:border-neutral-700">
                    <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                      {t('controlPanel.sip.associateNew')}
                    </label>
                    
                    <SIPAssociationForm
                      trunkNumbers={trunkNumbers}
                      scriptPlans={scriptPlans}
                      onAssociate={onAssociateNumber}
                      onRefreshTrunkNumbers={onRefreshTrunkNumbers}
                      onRefreshScriptPlans={onRefreshScriptPlans}
                      onCreateScriptPlan={onCreateScriptPlan}
                      onUpdateScriptPlan={onUpdateScriptPlan}
                    />
                  </div>
                </div>
              </motion.div>
...
// SIP关联表单组件
interface SIPAssociationFormProps {
  trunkNumbers: Array<{id: number, number: string, direction: string, status: string}>
  scriptPlans: Array<{id: number, name: string, scriptId?: string, scriptConfig?: string}>
  onAssociate: (numberId: number, scriptPlanId: number) => void
  onRefreshTrunkNumbers: () => void
  onRefreshScriptPlans: () => void
  onCreateScriptPlan?: (name: string, description: string, scriptId?: string, scriptConfig?: string) => Promise<void>
  onUpdateScriptPlan?: (id: number, name: string, description: string, scriptId?: string, scriptConfig?: string) => Promise<void>
...
const SIPAssociationForm: React.FC<SIPAssociationFormProps> = ({
  trunkNumbers,
  scriptPlans,
  onAssociate,
  onRefreshTrunkNumbers,
  ...
  const [selectedScriptPlanId, setSelectedScriptPlanId] = useState<number | ''>('')
  const [showCreateModal, setShowCreateModal] = useState(false)
  const [showEditModal, setShowEditModal] = useState(false)
  const [editingPlan, setEditingPlan] = useState<{id: number, name: string, description: string, scriptId?: string, scriptConfig?: string} | null>(null)
  const [newPlanName, setNewPlanName] = useState('')
  const [newPlanDescription, setNewPlanDescription] = useState('')
  const [newPlanScriptId, setNewPlanScriptId] = useState('')
  const [creating, setCreating] = useState(false)
  const [updating, setUpdating] = useState(false)

  const handleAssociate = () => {
    if (selectedNumberId && selectedScriptPlanId) {
      onAssociate(Number(selectedNumberId), Number(selectedScriptPlanId))
      setSelectedNumberId('')
      setSelectedScriptPlanId('')
    }
  }

  const handleCreateScriptPlan = async () => {
    if (!newPlanName.trim() || !onCreateScriptPlan) return
    
    setCreating(true)
    try {
      await onCreateScriptPlan(
        newPlanName.trim(), 
        newPlanDescription.trim(), 
        newPlanScriptId.trim() || undefined
      )
      ...
      console.error('创建脚本计划失败:', err)
    } finally {
      setCreating(false)
    }
  }

  const handleEditScriptPlan = (plan: {id: number, name: string, description?: string, scriptId?: string, scriptConfig?: string}) => {
    setEditingPlan({
      id: plan.id,
      name: plan.name,
      description: plan.description || '',
      scriptId: plan.scriptId,
      scriptConfig: plan.scriptConfig
    })
    setNewPlanName(plan.name)
    setNewPlanDescription(plan.description || '')
    setNewPlanScriptId(plan.scriptId || '')
    setShowEditModal(true)
  }

  const handleUpdateScriptPlan = async () => {
    if (!editingPlan || !onUpdateScriptPlan) return
    
    setUpdating(true)
    try {
      await onUpdateScriptPlan(
        editingPlan.id,
        newPlanName.trim(),
        newPlanDescription.trim(),
        newPlanScriptId.trim() || undefined
      ...
      console.error('更新脚本计划失败:', err)
    } finally {
      setUpdating(false)
    }
  }

  const activeNumbers = trunkNumbers.filter(n => n.status === 'active')

  return (
    <div className="space-y-3">
      <div className="space-y-2">
        <label className="text-xs text-gray-500 dark:text-gray-400">{t('controlPanel.sip.selectTrunk')}</label>
        <div className="flex gap-2">
          <select
            value={selectedNumberId}
            onChange={(e) => setSelectedNumberId(e.target.value ? Number(e.target.value) : '')}
            className="flex-1 p-2 text-sm border rounded-lg focus:ring-2 focus:ring-purple-500 dark:bg-neutral-700 dark:border-neutral-600"
          >
            <option value="">{t('controlPanel.sip.selectTrunkPlaceholder')}</option>
            {activeNumbers.map((num) => (
              <option key={num.id} value={num.id}>
                {num.number} ({num.direction === 'inbound' ? t('controlPanel.sip.inbound') : t('controlPanel.sip.outbound')})
              </option>
            ))}
          </select>
          <button
            onClick={onRefreshTrunkNumbers}
            className="px-3 py-2 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
            title={t('controlPanel.sip.refreshNumbers')}
          >
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <label className="text-xs text-gray-500 dark:text-gray-400">{t('controlPanel.sip.selectScript')}</label>
          {onCreateScriptPlan && (
            <button
        ...
        <div className="flex gap-2">
          <div className="flex-1 flex gap-2">
            <select
              value={selectedScriptPlanId}
              onChange={(e) => setSelectedScriptPlanId(e.target.value ? Number(e.target.value) : '')}
              className="flex-1 p-2 text-sm border rounded-lg focus:ring-2 focus:ring-purple-500 dark:bg-neutral-700 dark:border-neutral-600"
            >
              <option value="">{t('controlPanel.sip.selectScriptPlaceholder')}</option>
              {scriptPlans.map((plan) => {
                const p = plan as {id: number, name: string, scriptId?: string, scriptConfig?: string}
                return (
                  <option key={plan.id} value={plan.id}>
                    {plan.name} {p.scriptConfig ? `(${t('controlPanel.sip.custom')})` : p.scriptId ? `(${p.scriptId})` : ''}
                  </option>
                )
              })}
            </select>
            {selectedScriptPlanId && (
              <button
                onClick={() => {
                  const plan = scriptPlans.find(p => p.id === Number(selectedScriptPlanId))
                  if (plan) handleEditScriptPlan(plan)
                }}
                className="px-3 py-2 text-sm text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 border border-purple-300 dark:border-purple-600 rounded-lg hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors"
                title={t('controlPanel.sip.editPlan')}
              >
          ...
          <button
            onClick={onRefreshScriptPlans}
            className="px-3 py-2 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
            title={t('controlPanel.sip.refreshPlans')}
          >
            <RefreshCw className="w-4 h-4" />
          </button>
        </div>
      ...
      {/* 创建脚本计划模态框 */}
      {showCreateModal && onCreateScriptPlan && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{t('controlPanel.sip.createPlanTitle')}</h3>
              <button
                onClick={() => {
                  setShowCreateModal(false)
                  setNewPlanName('')
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  {t('controlPanel.sip.planName')} <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  value={newPlanName}
                  ...
                  className="w-full p-2 text-sm border rounded-lg focus:ring-2 focus:ring-purple-500 dark:bg-neutral-700 dark:border-neutral-600"
                  placeholder={t('controlPanel.sip.planNamePlaceholder')}
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  {t('controlPanel.sip.planDescription')}
                </label>
                <textarea
                  value={newPlanDescription}
                  onChange={(e) => setNewPlanDescription(e.target.value)}
                  className="w-full p-2 text-sm border rounded-lg focus:ring-2 focus:ring-purple-500 dark:bg-neutral-700 dark:border-neutral-600"
                  placeholder={t('controlPanel.sip.planDescriptionPlaceholder')}
                  rows={3}
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  {t('controlPanel.sip.scriptId')} <span className="text-gray-500 text-xs">{t('controlPanel.sip.scriptIdOptional')}</span>
                </label>
                <input
                  type="text"
                  value={newPlanScriptId}
                  onChange={(e) => setNewPlanScriptId(e.target.value)}
                  className="w-full p-2 text-sm border rounded-lg focus:ring-2 focus:ring-purple-500 dark:bg-neutral-700 dark:border-neutral-600"
                  placeholder={t('controlPanel.sip.scriptIdPlaceholder')}
                />
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  {t('controlPanel.sip.scriptIdHint')}
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  {t('controlPanel.sip.scriptConfig')}
                </label>
                <button
                  onClick={() => {
                    // 先创建计划，然后跳转到构建器
                    if (newPlanName.trim()) {
                      handleCreateScriptPlan()
                      setTimeout(() => {
                  ...
                  className="w-full p-3 border-2 border-dashed border-purple-300 dark:border-purple-600 rounded-lg hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors flex items-center justify-center gap-2 text-purple-600 dark:text-purple-400"
                >
                  <ExternalLink className="w-4 h-4" />
                  <span>{t('controlPanel.sip.useBuilder')}</span>
                </button>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  {t('controlPanel.sip.useBuilderHint')}
                </p>
              </div>
            </div>
            <div className="flex gap-2 mt-6">
              ...
              >
                {t('controlPanel.sip.cancel')}
              </button>
              <button
                onClick={handleCreateScriptPlan}
                disabled={!newPlanName.trim() || creating}
                className="flex-1 px-4 py-2 text-sm bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:bg-gray-300 disabled:text-gray-500 disabled:cursor-not-allowed transition-colors"
              >
                {creating ? t('controlPanel.sip.creating') : t('controlPanel.sip.create')}
              </button>
            </div>
          </div>
        </div>
      ...
      {/* 编辑脚本计划模态框 */}
      {showEditModal && editingPlan && onUpdateScriptPlan && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-4xl w-full p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{t('controlPanel.sip.editPlanTitle')}</h3>
              <button
                onClick={() => {
                  setShowEditModal(false)
                  setEditingPlan(null)
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <button
                  onClick={() => {
                    navigate(`/script-builder/${editingPlan.id}`)
                  }}
                  className="w-full p-3 border-2 border-dashed border-purple-300 dark:border-purple-600 rounded-lg hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors flex items-center justify-center gap-2 text-purple-600 dark:text-purple-400"
                >
                  <ExternalLink className="w-4 h-4" />
                  <span>{t('controlPanel.sip.editBuilder')}</span>
                </button>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  {t('controlPanel.sip.editBuilderHint')}
                </p>
              </div>
            </div>
            <div className="flex gap-2 mt-6">
              ...
              <button
                onClick={handleUpdateScriptPlan}
                disabled={!newPlanName.trim() || updating}
                className="flex-1 px-4 py-2 text-sm bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:bg-gray-300 disabled:text-gray-500 disabled:cursor-not-allowed transition-colors"
              >
                {updating ? t('controlPanel.sip.saving') : t('controlPanel.sip.save')}
              </button>
            </div>
          </div>
        </div>
      ...
      <button
        ...
        disabled={!selectedNumberId || !selectedScriptPlanId}
        className="w-full px-4 py-2 text-sm bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:bg-gray-300 disabled:text-gray-500 disabled:cursor-not-allowed transition-colors"
      >
        {t('controlPanel.sip.associate')}
      </button>
    </div>
  )
}
