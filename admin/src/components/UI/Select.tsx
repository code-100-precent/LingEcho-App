...
const SelectContent: React.FC<SelectContentProps & { onItemClick?: (value: string) => void; selectedValue?: string }> = ({
    ...
    return (
        <div className={cn(
            'absolute top-full left-0 right-0 z-[100] mt-1.5 max-h-60 overflow-auto rounded-lg border border-gray-200 bg-white py-1.5 shadow-xl ring-1 ring-black ring-opacity-5 transition-all duration-200',
            className
        )}>
            {React.Children.map(children, (child) => {
